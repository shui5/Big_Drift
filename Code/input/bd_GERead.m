function MRS_struct = bd_GERead(fname)
...
chkRev = {'14.3', '16', '24', '20.006', '20.007', '26.002', '27.001'}; % GERead mods tested with these revisions only
...
MRS_struct.p.nreceivers = (stop_recv - start_recv) + 1;
...
data_elements = data_elements * totalframes * MRS_struct.p.nreceivers;
...
if nechoes == 1
    ...
    ShapeData = reshape(raw_data, [2 MRS_struct.p.npoints(ii) totalframes MRS_struct.p.nreceivers]);
...
%     Y1 = permute(repmat(Y1, [1 MRS_struct.p.npoints(ii) 2 MRS_struct.p.nreceivers]), [3 2 1 4]);
...
MetabData = MetabData .* repmat([1; 1i], [1 MRS_struct.p.npoints(ii) totalframes MRS_struct.p.nreceivers]);
MetabData = squeeze(sum(MetabData,1));
MetabData = permute(MetabData, [3 1 2]);

% Need to correct phasing
corrPhi              = ones(size(MetabData));
corrPhi(:,:,2:2:end) = -corrPhi(:,:,2:2:end);
MetabData            = MetabData .* corrPhi;

% Perform coil combination
[~,ind]         = max(abs(mean(MetabData,3)),[],2);
ind             = mode(ind);
maxPoint        = conj(MetabData(:,ind,:));
...
MetabData       = MetabData .* maxPoint;
MRS_struct.fids = squeeze(sum(MetabData,1));
MRS_struct.fids = MRS_struct.fids/1e11; % rescale, otherwise numbers blow up
