function MRS_struct = bd_SiemensTwixRead(fname)
%% function MRS_struct = bd_SiemensTwixRead(MRS_struct, fname, fname_water)
...
MRS_struct.p.TR(ii)                     = MetabHeader.TR(1); % MM (201103)
...
MRS_struct.p.nreceivers(ii)             = MetabHeader.sqzSize(contains(MetabHeader.sqzDims,'Cha'));
...
MRS_struct.fids = double(conj(squeeze(sum(MetabData,1))));
