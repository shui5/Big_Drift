function kspace = bd_loadRawKspace(listordatafile, varargin)
...
fid = fopen(listfile);
if (fid == -1)
    error('MATLAB:bd_loadRawKspace:wrong_inputfile','Please provide a valid list file');
end
nrofheaderlines = 0;
currentline = '#';
...
while any(strcmp(currentline(1),{'#','.'}))
   ...
   if strcmp(currentline(1),'.')
       ...
       if isfield(kspace_properties,fieldname) % for multipliple occasions...
           kspace_properties.(fieldname)(end+1,:) = str2double(currentline(tmpi+1:end));
       else
           kspace_properties.(fieldname) = str2double(currentline(tmpi+1:end));
       end
